...
let
  ...
  greeters = [ "agreety" "gtkgreet" "dlm" "wlgreet" "tuigreet" ];
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc."greetd/config.toml" = {
      source = (pkgs.formats.toml {}).generate "config.toml" {
        ...
        default_session = {
          command = if cfg.command == null then {
  ...
  meta.maintainers = with lib.maintainers; [ luc65r ];
}
